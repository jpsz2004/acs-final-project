...
class Worker(threading.Thread):
    ...
    def run(self) -> None:
        while not self._stop_event.is_set():
            ...
            try:
                ...
                # Publicar evento exactamente una vez cuando el job pasa a completed.
                updated = self._repo.get(command.job_id)
                if updated.status == JobStatus.completed and self._repo.try_mark_notified(updated.job_id):
                    results_url = self._build_results_url(updated.job_id)
                    self._event_bus.publish(JobCompletedEvent(job_id=updated.job_id, user_id=updated.user_id, results_url=results_url))
    ...
    def _build_results_url(self, job_id: str) -> str:
        if self._public_base_url:
            return f"{self._public_base_url.rstrip('/')}/jobs/{job_id}"
